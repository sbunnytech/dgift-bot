...
function getOwnerVariants(ownerNumber) {
  const num = toNumber(ownerNumber)
  if (!num) return []
  return [
    `${num}@s.whatsapp.net`,
    `${num}@lid`,
...
export async function handleMessages(sock, m, botSettings) {
  try {
    ...
    // Owner is only sock.user.id. Supabase owner_number is ignored.
    const botNumber = sock.user?.id?.split(':')[0] + '@s.whatsapp.net'
    const ownerVariants = getOwnerVariants(botNumber)
    const senderNumber = toNumber(sender)
    const isOwner = ownerVariants.includes(sender) || ownerVariants.includes(senderNumber)
    ...
    if (!allowed) {
      await sock.sendMessage(from, { text: 'Access Denied. You do not have permission to use this command.' }, { quoted: msg })
      return
    }
