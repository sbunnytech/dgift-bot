...
async function loadThemeEngine(botSettings = {}) {
  try {
    const themesPath = join(__dirname, '..', 'themes')

    if (!existsSync(themesPath)) {
      console.log('[THEME] themes/ folder not found')
      return
    }
    ...
    if (!files.length) {
      console.log('[THEME] No themes found')
      return
    }
    ...
    const imported = await import(
      pathToFileURL(join(themesPath, found)).href
    )

    activeTheme = imported.default || imported

    console.log(
      `[THEME] Loaded: ${basename(found, '.js')}`
    )
...
export function formatBox(type = 'default', data = {}) {
  try {
    if (
      activeTheme &&
      typeof activeTheme[type] === 'function'
    ) {
      return activeTheme[type](data)
    }

    if (
      activeTheme &&
      typeof activeTheme.box === 'function'
    ) {
      return activeTheme.box(type, data)
    }
...
function getOwnerVariants(sock, botSettings = {}) {
  ...
  try {
    if (sock?.authState?.creds?.me?.id) {
      variants.add(
        normalizeJid(sock.authState.creds.me.id)
      )

      variants.add(
        toNumber(sock.authState.creds.me.id)
      )
    }
  } catch {}
  ...
  try {
    if (sock?.authState?.creds?.me?.lid) {
      variants.add(
        normalizeJid(sock.authState.creds.me.lid)
      )

      variants.add(
        toNumber(sock.authState.creds.me.lid)
      )
    }
  } catch {}
  ...
  try {
    safeArray(botSettings?.admin_numbers)
      .forEach(v => {
  ...
  for (const item of items) {
    ...
    try {
      ...
      const cmdData = {
        ...
        category:
          imported.category || 'General',
        desc:
          imported.desc || 'No description',
        restricted:
...
export async function initializeRouter(
  ...
  try {
    ...
    await loadThemeEngine(botSettings)
...
export async function handleMessages(
  ...
  try {
    ...
    const vipNumbers =
      safeArray(botSettings?.vip_numbers)

    const isVIP =
      vipNumbers.includes(senderNumber)
