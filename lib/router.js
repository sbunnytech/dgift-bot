...
function normalizeJid(jid) {
  ...
  // Remove device suffix: 2547...:5@s.whatsapp.net -> 2547...@s.whatsapp.net
  jid = jid.split(':')[0]
...
function isVIP(jid) {
  ...
  return VIP_NUMBERS.includes(number)
}
  ...
  try {
    ...
    if (isGroup) {
      try {
        ...
        const userParticipant = participants.find(p => normalizeJid(p.id) === sender)
    ...
    // VIP bypass: force admin and owner rights for VIP numbers
    ...
    if (commands.has(providedName)) {
      ...
      console.log(`[CMD] ${providedName} from ${pushName} [${sender}] ${isVIPUser? '[VIP]' : ''}`)
