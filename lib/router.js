...
export async function initializeRouter(botSettings) {
  ...
  isLoaded = true
  const brand = botSettings.brand_name // FIX: Ondoa || 'Bot' fallback
...
export async function handleMessages(sock, m, botSettings) {
  try {
    ...
    const pushName = msg.pushName // FIX: Ondoa || 'User' fallback
    ...
    const vipNumbers = botSettings.vip_numbers // FIX: Ondoa || []
    ...
    const restrictedCategories = botSettings.restricted_categories // FIX: Ondoa || ['Settings', 'Auto', 'Anti', 'Owner']
    const stableMode = botSettings.stable_mode // FIX: Ondoa!== false
    const antiBanMode = botSettings.anti_ban_mode // FIX: Ondoa === true

    // FIX 1: Wrap supabase for instance_id force
    const wrappedSettings = {...botSettings, supabase: wrapSupabase(botSettings.supabase, botSettings.instance_id) }

    // FIX 2: Observers sasa wanapata wrappedSettings pia, si botSettings ya asili
      ...
      try {
        await observer.run(
          sock,
          {
            ...
            stableMode,
            antiBanMode,
            botSettings: wrappedSettings // FIX: Tumia wrapped
          },
          wrappedSettings // FIX: Tumia wrapped
        )
      } catch (err) {
    ...
    const cmdContext = {
      ...
      stableMode,
      antiBanMode,
      botSettings: wrappedSettings // FIX 3: Commands pia zinapata wrapped
    }
    ...
    const ownerMode = botSettings.owner_mode
    const publicMode = botSettings.public_mode
    const privatePublicMode = botSettings.private_public_mode
