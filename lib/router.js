...
async function loadThemeEngine(
  ...
  try {
    ...
    console.log(
      `[THEME] Loaded: ${activeThemeName} | Instance: ${INSTANCE_ID}`
    )
...
export async function refreshTheme(
  ...
  try {
    ...
    if (
      nextTheme.toLowerCase() ===
      activeThemeName.toLowerCase()
    ) {
      return
    }

    console.log(
      `[THEME] Switching ${activeThemeName} -> ${nextTheme}`
    )
...
function watchThemesFolder() {
  try {
    ...
    if (currentThemeWatcher) {
      return
    }

    currentThemeWatcher = watch(
      themesPath,
      async (eventType, filename) => {
        try {
          ...
          console.log(
            `[THEME WATCH] ${eventType}: ${filename}`
          )

          await loadThemeEngine({
    ...
    console.log(
      '[THEME WATCH] Live watcher active'
    )
...
export async function handleMessages(
 ...
 botSettings = {}
  ...
  try {
    ...
    // =========================================
    // REALTIME THEME CHECK
    // =========================================

    if (
      botSettings?.theme_name &&
      botSettings.theme_name !== activeThemeName
    ) {
      await refreshTheme(botSettings)
    }
