...
export const alias = ['mscopilot']
export const category = 'AI'
export const desc = 'Chat with Copilot'

export default async function copilot(sock, { msg, from }, botSettings) {
  try {
    ...
    if (!prompt) {
      return sock.sendMessage(from, { text: '❌ Usage:.copilot hello' }, { quoted: msg })
    }
    ...
    const systemPrompt = `You are Microsoft Copilot, a smart AI assistant created by Microsoft.

Rules:
1. Answer in the user's language. Match exactly.
2. Keep replies short, 2-3 lines max unless user asks for details.
3. Be direct, helpful, professional, and clear.
4. You are Microsoft Copilot, created by Microsoft.
5. If asked who made you: "Microsoft"
...
8. No disclaimers unless dangerous.`
    ...
    console.log('Copilot command error:', error.message)
