...
│
│ Usage:
│ ${botSettings.prefix}autoread on global
│ ${botSettings.prefix}autoread off group
╰⊷ *${botSettings.botname}*`
      }, { quoted: msg })
    }
    ...
    if (!validOptions.includes(action)) {
      await sock.sendMessage(from, { react: { text: '❌', key: msg.key } })
      return await sock.sendMessage(from, { text: `> Invalid. Use: on/off` }, { quoted: msg })
    }
    ...
    const { error } = await supabase
     .from('b_settings')
     .upsert({ id: targetJid, autoread: newValue, updated_at: new Date().toISOString() }, { onConflict: 'id' })
